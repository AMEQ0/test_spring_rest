...
public class BibliotecaController {
	...
	public ResponseEntity<Page<Biblioteca>> listarBiblioteca(Pageable pageable){
		return ResponseEntity.ok(repoBiblio.findAll(pageable));
	}

	@PostMapping
	public ResponseEntity<Biblioteca> guardarBiblioteca(@Valid @RequestBody Biblioteca biblioteca) {
		Biblioteca bibliotecaGuardada = repoBiblio.save(biblioteca);
	...
	public ResponseEntity<Biblioteca> actualizarBiblioteca(@PathVariable Integer id,
		...
		Optional<Biblioteca> bibliotecaOptional = repoBiblio.findById(id);

		if (!bibliotecaOptional.isPresent()) {
			return ResponseEntity.unprocessableEntity().build();
		}
		biblioteca.setId(bibliotecaOptional.get().getId());
		repoBiblio.save(biblioteca);
	...
	public ResponseEntity<Biblioteca> eliminarBiblioteca(@PathVariable Integer id) {
		...
		repoBiblio.delete(bibliotecaOptional.get());
		return ResponseEntity.noContent().build();		
	}
